...
public class SeckillServiceImpl implements SeckillService {
    ...
    public Exposer exportSeckillUrl(long seckillId) {
       /* Seckill seckill = seckillDao.queryById(seckillId);  // 未优化
        if(seckill == null){
            return new Exposer(false,seckillId);
        }*/

        // 优化点：超时缓存，减低数据库的访问量
        // 超时的基础上维护一致性，如果临近秒杀 商品 除了问题 一般是不允许修改 而是 废弃掉 重新建
        Seckill seckill = redisDao.getSeckill(seckillId);  // redis 中获取
        if (seckill == null) {
            seckill = seckillDao.queryById(seckillId);  // db 中获取
            if (seckill == null) {
                return new Exposer(false, seckillId);
            } else {
                redisDao.putSeckill(seckill);  // 添加到 redis
            }
        }
